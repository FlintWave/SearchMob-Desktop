...
class SettingsDialog(QDialog):
    ...
    def __init__(
        ...
        self._tabs.addTab(self._build_local_ai_tab(), "Local AI")
    ...
    def _build_local_ai_tab(self) -> QWidget:
        ...
        intro = QLabel(
            ...
            "computer. It uses only your search results and never leaves your device. You need a "
            "local model server running: Ollama (port 11434) or LM Studio (port 1234)."
        )
        intro.setWordWrap(True)
        intro.setProperty("role", "muted")
        layout.addWidget(intro)

        self._llm_enable = QCheckBox("Show a local AI answer above results")
        self._llm_enable.setChecked(self._prefs.llm_enabled)
        self._llm_enable.toggled.connect(self._on_llm_enabled_toggled)
        layout.addWidget(self._llm_enable)

        detect_row = QHBoxLayout()
        self._detect_btn = QPushButton("Detect local models")
        self._detect_btn.clicked.connect(self._on_detect_models)
        detect_row.addWidget(self._detect_btn)
        detect_row.addWidget(QLabel("Model:"))
        self._llm_combo = QComboBox()
        self._llm_combo.setEnabled(False)
        # Show the saved selection (if any) before a detect run so it is not blank.
        if self._prefs.llm_model:
            label = f"{self._prefs.llm_model}"
            self._llm_combo.addItem(label, (self._prefs.llm_base_url, self._prefs.llm_model))
            self._llm_combo.setEnabled(True)
        self._llm_combo.currentIndexChanged.connect(self._on_llm_model_changed)
        detect_row.addWidget(self._llm_combo, stretch=1)
        layout.addLayout(detect_row)

        self._llm_status = QLabel(
            "Pick a model after detecting one." if not self._prefs.llm_model else "Model selected."
        )
    ...
    def _on_llm_enabled_toggled(self, checked: bool) -> None:
        self._save(replace(self._prefs, llm_enabled=checked))

    def _on_detect_models(self) -> None:
        self._detect_btn.setEnabled(False)
        self._llm_status.setText("Detecting local model servers ...")
    ...
    def _on_detect_failed(self, _message: str) -> None:
        ...
        self._llm_status.setText("Detection failed. Is a local model server running?")

    def _on_models_detected(self, payload: object) -> None:
        self._detect_btn.setEnabled(True)
        backends = payload if isinstance(payload, list) else []
        pairs: list[tuple[str, str]] = []
        # Block the change signal while repopulating so it does not persist a half-built selection.
        ...
        for backend in backends:
            ...
            for model in backend.models:
                self._llm_combo.addItem(f"{backend.name} — {model}", (backend.base_url, model))
                pairs.append((backend.base_url, model))
        ...
        if not pairs:
            ...
            return
        self._llm_combo.setEnabled(True)
        saved = self._llm_combo.findData((self._prefs.llm_base_url, self._prefs.llm_model))
        self._llm_combo.setCurrentIndex(saved if saved >= 0 else 0)
        # Persist whatever is now selected so a fresh detect immediately wires up a usable model.
        self._on_llm_model_changed()
        self._llm_status.setText(f"Found {len(pairs)} model(s). Pick one to use.")

    def _on_llm_model_changed(self) -> None:
        data = self._llm_combo.currentData()
        if isinstance(data, tuple) and len(data) == 2:
            base_url, model = data
            self._save(replace(self._prefs, llm_base_url=str(base_url), llm_model=str(model)))
