...
from searchmob_desktop.i18n import tr, trn
...
class HistoryDialog(QDialog):
    ...
    def __init__(self, history_store: HistoryStore, parent: QWidget | None = None) -> None:
        ...
        import_btn = QPushButton(tr("Import from file..."))
    ...
    def _refresh(self) -> None:
        ...
        if not enabled:
            self._status.setText(
                tr("Search history is off. Turn it on in Settings to record queries.")
            )
        elif not entries:
            self._status.setText(tr("No saved searches yet."))
        else:
            n = len(entries)
            self._status.setText(trn(n, "{n} saved search.", "{n} saved searches."))
    ...
    def _on_clear_all(self) -> None:
        confirm = QMessageBox.question(
            ...
            tr("Delete every saved search on this device? This cannot be undone."),
    ...
    def _on_export(self) -> None:
        ...
        if not entries:
            QMessageBox.information(
                self, tr("Nothing to export"), tr("There is no history to export.")
            )
            return
        path, _ = QFileDialog.getSaveFileName(
            self, tr("Export history"), "searchmob-history.json", "JSON files (*.json)"
        )
        if not path:
            return
        ...
        QMessageBox.information(
            self,
            tr("History exported"),
            trn(n, "{n} entry exported.", "{n} entries exported."),
        )
    ...
    def _on_import(self) -> None:
        if not self._history.enabled:
            QMessageBox.information(
                self,
                tr("History is off"),
                tr("Turn on search history in Settings before importing."),
            )
            return
        path, _ = QFileDialog.getOpenFileName(self, tr("Import history"), "", "JSON files (*.json)")
        if not path:
            return
        ...
        QMessageBox.information(
            self,
            tr("History imported"),
            trn(added, "{n} new entry imported.", "{n} new entries imported."),
        )
        ...
        try:
            ...
            if os.path.getsize(path) > _MAX_IMPORT_BYTES:
                QMessageBox.warning(
                    self,
                    tr("File too large"),
                    tr("That history file is too large to import (limit 16 MiB)."),
                )
                return None
            ...
            QMessageBox.warning(
                self, tr("Import failed"), tr("Could not read the file: {exc}", exc=exc)
            )
            return None
        if not isinstance(raw, list):
            QMessageBox.warning(self, tr("Import failed"), tr("The file is not a history export."))
            return None
